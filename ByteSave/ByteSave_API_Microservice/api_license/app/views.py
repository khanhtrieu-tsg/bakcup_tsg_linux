...
client = MongoClient("mongodb://localhost:27017/ByteSave_Licenses")
db = client.projectDB
...
api.add_resource(Retrieve, '/hello')
...
#     return jsonify({'data': data})
#
# @app.route('/them-moi', methods=['GET', 'POST'])
# def add_or_edit():
...
#
# @app.route('/xoa', methods=['POST','GET'])
# def delete():
#     try:
#         form = request.form
#         if form.get('id') != '':
...
#
# @app.route('/thay-doi-so-luong', methods=['POST','GET'])
# def changequantity():
#     try:
#         form = request.form
...
#
# @app.route('/gia-han/<id>', methods=['POST','GET'])
# def giahan(id):
#     try:
#         form = request.form
#         if id != '':
...
#
# @app.route('/dung-hoat-dong', methods=['POST','GET'])
# def stop():
#     try:
#         form = request.form
#         if form.get('idLicense') != '':
...
#
# @app.route('/kiem-tra-ban-quyen/<MA_BAN_QUYEN>/<DIA_CHI_MAC>', methods=['POST','GET'])
...
#     return jsonify({'status': 'OK', 'msg': 'Mã bản quyền hợp lệ!'})
#
# @app.route('/kich-hoat-ban-quyen/<MA_BAN_QUYEN>/<DIA_CHI_MAC>/<ID_PHIEN_BAN>/<IP_PRIVATE>/<IP_PUBLIC>/<OS>', methods=['POST','GET'])
